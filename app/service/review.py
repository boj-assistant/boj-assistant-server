...
from app.service.crawler import get_solution_from_boj
from app.service.web import get_web_search
...
async def get_solution(problem_id: int, language: str):
    
    solution = await get_solution_from_boj(problem_id, language)

    if solution is None:
        logger.info(f"Solution not found in BOJ, searching web")
        solution = await get_web_search(problem_id, language)

    if solution is None:
        logger.info(f"Solution not found in web search")
        return None
...
async def get_problem_solution(problem_id: int, language: Literal["java8", "node.js", "python3", "c++17"]) -> str:
    ...
    if exists:
        ...
        if solution is None:
            solution = await get_solution(problem_id, language)
            if solution is None:
                return None
        ...
        (title, tags), solution = await asyncio.gather(
            ...
            get_solution(problem_id, language),
        )
...
async def get_review(request: ReviewCreateRequest) -> ReviewResponse:
    ...
    if correct_solution is None:
        return ReviewResponse(review="Cannot find correct solution from BOJ or web search")
